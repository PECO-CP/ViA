...
status = bfmatlab.bfCheckJavaPath(autoloadBioFormats);
assert(status, ['Missing Bio-Formats library. Either add bioformats_package.jar '...
    'to the static Java path or add it to the Matlab path.']);

% Prompt for a file if not input
if nargin == 0 || exist(id, 'file') == 0
  [file, path] = uigetfile(bfmatlab.bfGetFileExtensions, 'Choose a file to open');
  id = [path file];
  if isequal(path, 0) || isequal(file, 0), return; end
end

% Initialize logging
bfmatlab.bfInitLogging();

% Get the channel filler
r = bfmatlab.bfGetReader(id, stitchFiles);
...
for s = 1:numSeries
    h = findobj('Tag','Status Bar');
    txt_nf = ['Reading series #' num2str(s) ' '];
    txt = ['\n' txt_nf '\n\t'];
    fprintf(txt);
    if ~isempty(h)
        h.String = txt_nf;
    ...
    for i = 1:numImages
        ...
        arr = bfmatlab.bfGetPlane(r, i, varargin{:});
